...
class KeywordMonitor:
    ...
    def _load_keyword_data(self) -> Dict[str, Any]:
        ...
        default_data = {"entities": {}, "last_updated": datetime.datetime.now().isoformat()}
        
        if not self.data_file.exists():
            logger.info(f"No existing keyword data file found at {self.data_file}. Creating new database.")
        ...
        try:
            with open(self.data_file, 'r', encoding='utf-8') as f:
                ...
                if not file_content:  # Empty file
                    logger.warning(f"Empty keyword data file at {self.data_file}. Creating new database.")
                    self._save_keyword_data(default_data)
                    return default_data
    ...
    def extract_entities(self, headlines: List[str]) -> Dict[str, str]:
        ...
        if not self.nlp:
            ...
            logger.warning("Using fallback entity extraction method (spaCy model not available)")
