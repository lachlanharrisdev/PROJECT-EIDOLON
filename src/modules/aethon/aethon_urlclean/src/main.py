...
parser.add_argument(
    "-w",
    "--whitelist",
    help="only keep these extensions and extensionless urls",
    dest="whitelist",
    nargs="+",
)
parser.add_argument(
    "-b", "--blacklist", help="remove these extensions", dest="blacklist", nargs="+"
)
parser.add_argument(
    "-f", "--filters", help="additional filters, read docs", dest="filters", nargs="+"
)
...
ext_list = (
    tuple(clean_nargs(args.blacklist))
    if args.blacklist
    else tuple(
        (
            "css",
...
def process_url(url):
    ...
    new_params = (
        []
        if not params
        else [param for param in params.keys() if param not in params_seen]
    )
...
def main():
    if args.input_file:
        with open(
            args.input_file, "r", encoding="utf-8", errors="ignore"
        ) as input_file:
